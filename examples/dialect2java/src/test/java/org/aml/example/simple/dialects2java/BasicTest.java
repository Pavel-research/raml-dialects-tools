...
public class BasicTest extends TestCase {
	...
	@Test
	/**
	 * load from simple json and store to yaml then parse it from yaml again
	 */
	public void test4() {
		DialectRegistry.getDefault().registerClass(profileNode.class);
		profileNode profile = (profileNode) DialectRegistry.getDefault()
		...
		assertEquals(profile.getVersion(), "0.1-SNAPSHOT");
		try {
			String yaml = DialectRegistry.getDefault().storeAs(profile,YAML.class);
			try {
				profileNode parse = DialectRegistry.getDefault().parse(URI.create("/test.yaml"), profileNode.class,yaml);
				JSONObject jsonld = DialectRegistry.getDefault().toJSON(parse);
				JSONAssert.assertEquals(jsonld,
						new JSONObject(new JSONTokener(BasicTest.class.getResourceAsStream("/test-simple.json"))), true);
			} catch (Exception e) {
				fail();
